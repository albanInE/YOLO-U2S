...
坐标系转换系列函数
总共有四个坐标系：
1. 地面坐标系,dr_map,由adma定位或者dr算法得出 (mm)
2. 车辆坐标系,vehicle_map,车辆后轴中心坐标系 (mm)
3. 大地图坐标系,global_map,即地面坐标系,固定为某个时刻的车身坐标系 (cm)
4. 小图坐标系,local_map,某个时刻下的大地图截取部分 (cm)
"""



"""
根据车体的后轴中心坐标，计算超声波实时坐标
car_pos: (x, y ,rad)
snsID  : 0~11
return : uss_pos(x,y,angle)
"""
def cal_uss_pos(self, car_pos, snsID)->tuple:
    uss_x, uss_y, uss_a = UssPos[snsID]
    car_x, car_y, car_rad = car_pos
    cos_temp = math.cos(car_rad)
    sin_temp = math.sin(car_rad)

    #计算出汽车运动后的坐标
    t_x =  (uss_x*cos_temp-uss_y*sin_temp)+car_x
    t_y =  (uss_y*cos_temp+uss_x*sin_temp)+car_y
    t_angle =  uss_a + (car_rad/math.pi*180)

    return (int(t_x),int(t_y),int(t_angle))

"""
根据相对于车体后轴中心的相对坐标，计算出绝对坐标
"""
def __cal_parking_point_pos(self, car_pos, target_pos)->tuple:
    x, y = target_pos[0]
    x = x*1000
    y = y*1000
    car_x, car_y, car_rad = car_pos
    ...
    #计算出汽车运动后的坐标
    t_x =  (x*cos_temp-y*sin_temp) + car_x #真值点单位是m 换算成mm
    t_y =  (y*cos_temp+x*sin_temp) + car_y
